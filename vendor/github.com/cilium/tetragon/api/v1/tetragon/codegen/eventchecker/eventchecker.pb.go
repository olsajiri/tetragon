...
func (checker *ProcessLoaderChecker) Check(event *tetragon.ProcessLoader) error {
	...
	if checker.Pid != nil {
		if *checker.Pid != event.Pid {
			return fmt.Errorf("ProcessLoaderChecker: Pid has value %d which does not match expected value %d", event.Pid, *checker.Pid)
		}
	}
	if checker.Tid != nil {
		if *checker.Tid != event.Tid {
			return fmt.Errorf("ProcessLoaderChecker: Tid has value %d which does not match expected value %d", event.Tid, *checker.Tid)
		}
	}
...
// WithPid adds a Pid check to the ProcessLoaderChecker
func (checker *ProcessLoaderChecker) WithPid(check uint32) *ProcessLoaderChecker {
	checker.Pid = &check
	return checker
}

// WithTid adds a Tid check to the ProcessLoaderChecker
func (checker *ProcessLoaderChecker) WithTid(check uint32) *ProcessLoaderChecker {
	checker.Tid = &check
	return checker
}
