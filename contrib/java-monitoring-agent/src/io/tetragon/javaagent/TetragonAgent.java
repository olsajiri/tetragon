...
public final class TetragonAgent {
	...
	public static void premain(String agentArgs, Instrumentation instrumentation) {
		...
		try {
			ipc = JavaIPC.open(args.getOrDefault("socket", JavaIPC.DEFAULT_PATH));
		} catch (Exception e) {
		...
		System.err.printf("tetragon-java-agent: monitoring %s#%s via %s%n",
				wildcard(className), wildcard(methodName), args.getOrDefault("socket", JavaIPC.DEFAULT_PATH));
	}

	private static String argument(Map<String, String> args, String key) {
		String value = args.get(key);
		if (value == null) throw new IllegalArgumentException("-javaagent arguments must include class=<name>,method=<name>");
		return value;
	}

	private static String wildcard(String value) { return value.isEmpty() ? "*" : value; }

	private static Map<String, String> parseArgs(String input) {
		Map<String, String> result = new HashMap<>();
		if (input != null) for (String item : input.split(",")) {
	...
	public static void onMethodEntry(long methodId, String className, String methodName, String descriptor) {
		...
		if (producer != null) producer.submit(methodId, className, methodName, descriptor);
	}
	...
	static final class Transformer implements ClassFileTransformer {
		...
		Transformer(String targetClass, String targetMethod) { this.targetClass = targetClass; this.targetMethod = targetMethod; }
		@Override public byte[] transform(ClassLoader loader, String className, Class<?> redefining,
				ProtectionDomain domain, byte[] bytes) {
			if ((!targetClass.isEmpty() && !targetClass.equals(className)) || excluded(className)) return null;
			try {
		...
		private static boolean excluded(String className) {
			return className == null || className.startsWith("java/") || className.startsWith("jdk/") ||
	...
	private static final class Visitor extends ClassVisitor {
		private final String className, targetMethod;
		Visitor(ClassVisitor visitor, String className, String targetMethod) {
			super(Opcodes.ASM9, visitor); this.className = className; this.targetMethod = targetMethod;
		}
		@Override public MethodVisitor visitMethod(int access, String name, String descriptor,
				String signature, String[] exceptions) {
			MethodVisitor visitor = super.visitMethod(access, name, descriptor, signature, exceptions);
			if (visitor == null || (!targetMethod.isEmpty() && !targetMethod.equals(name)) || "<clinit>".equals(name)) return visitor;
			long id = methodId(className, name, descriptor);
			return new AdviceAdapter(Opcodes.ASM9, visitor, access, name, descriptor) {
				@Override protected void onMethodEnter() {
					visitLdcInsn(id); visitLdcInsn(className); visitLdcInsn(name); visitLdcInsn(descriptor);
					visitMethodInsn(INVOKESTATIC, "io/tetragon/javaagent/TetragonAgent", "onMethodEntry",
