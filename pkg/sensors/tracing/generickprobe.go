...
func filterMaps(load *program.Program, pinPath string, kprobeEntry *genericKprobe) []*program.Map {
	...
	if kprobeEntry != nil {
		if load.RetProbe {
			state = kprobeEntry.loadArgs.selectors.retrn
		} else {
			state = kprobeEntry.loadArgs.selectors.entry
		}
	}
...
func createMultiKprobeSensor(sensorPath string, multiIDs []idtable.EntryID) ([]*program.Program, []*program.Map, error) {
	...
	if len(multiRetIDs) != 0 {
		...
		filterMap := program.MapBuilderPin("filter_map", sensors.PathJoin(pinPath, "filter_map"), loadret)
		maps = append(maps, filterMap)
...
func getMapLoad(load *program.Program, gk *genericKprobe, index uint32) []*program.MapLoad {
	var ks *selectors.KernelSelectorState

	if load.RetProbe {
		ks = gk.loadArgs.selectors.retrn
	} else {
		ks = gk.loadArgs.selectors.entry
	}

	return selectorsMaploads(ks, gk.pinPathPrefix, index)
}
