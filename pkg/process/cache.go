...
func NewCache(
	...
	lruCache, err := lru.NewWithEvict(
		...
		func(_ string, evicted *ProcessInternal) {
			...
			pm.removePIDIndex(evicted.process)
...
func (pc *Cache) add(process *ProcessInternal) bool {
	...
	pc.addPIDIndex(process.process)
	if !evicted {
...
func (pc *Cache) addPIDIndex(process *tetragon.Process) {
...
func (pc *Cache) removePIDIndex(process *tetragon.Process) {
	if process == nil {
		return
	}
	if process.Pid == nil {
