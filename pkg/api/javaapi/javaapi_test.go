...
func TestPreparePacketAuthenticatesPeer(t *testing.T) {
	...
	require.NoError(t, PreparePacket(data.Bytes(), 4321))
	require.Equal(t, uint32(4321), binary.LittleEndian.Uint32(data.Bytes()[16:20]))
	require.Equal(t, uint64(1234), binary.LittleEndian.Uint64(data.Bytes()[24:32]))
}
...
func TestPreparePacketRejectsInvalidUTF8(t *testing.T) {
	...
	data[44] = 0xff
	require.Error(t, PreparePacket(data, 1))
}
