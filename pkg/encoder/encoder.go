...
func (p *CompactEncoder) EventToString(response *tetragon.GetEventsResponse) (string, error) {
	...
	case *tetragon.GetEventsResponse_ProcessLoader:
		...
		return CapTrailorPrinter(fmt.Sprintf("%s %s %s %s %d", event, processInfo,
			hex.EncodeToString(loader.Binary.Buildid), loader.Binary.Path, len(loader.Binary.Buildid)), caps), nil
