...
/* Looks up this invocation's process_call_heap entry, seeding a fresh one
 * from the read-only zero template on first touch. Needed because
 * map_update_elem() requires a value to copy from, and one this size can't
 * live on the BPF stack.
 */
FUNC_INLINE void *uprobe_heap_get(void)
{
	heap_key_t key = heap_key();
	void *val = map_lookup_elem(&process_call_heap, &key);
	...
	return map_lookup_elem(&process_call_heap, &key);
}
