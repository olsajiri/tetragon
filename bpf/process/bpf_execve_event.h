...
execve_event_size(struct bpf_raw_tracepoint_args *ctx __arg_ctx,
		  struct execve_rb_data *d __arg_nonnull)
{
	struct linux_binprm *bprm = (struct linux_binprm *)ctx->args[2];
	struct task_struct *task = (struct task_struct *)get_current_task();
	struct mm_struct *mm = NULL;
...
FUNC_INLINE __u32
execve_rb_read_data(struct msg_execve_event *event, struct bpf_dynptr *ptr,
		    struct execve_rb_data *d)
{
	struct msg_process *p = &event->process;
...
{
	struct execve_rb_data data = {};
	struct msg_execve_event *event;
	struct bpf_dynptr ptr;
	__u64 size;
	__u32 off;

	size = execve_event_size(ctx, &data);
	...
	off = execve_rb_read_data(event, &ptr, &data);
