...
{{< caution >}}
By default Tetragon does not expose the kernel addresses, you need to enable the
flag `--stack-traces-kernel-addresses` to get the addresses along the rest.
...
{{< note >}}
Compact output will display missing addresses as `0x0`, see the
above note on `--stack-traces-kernel-addresses` for more info.
{{< /note >}}
